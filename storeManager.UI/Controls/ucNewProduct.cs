...
namespace storeManager.UI.Controls
{
    public partial class ucNewProduct : UserControl
    {
        ...
        private ProductService _pruduct;
        private ProductLocationService _prodLoc;
        ...
        IEnumerable<Location> location = new LocationService().GetAll();
        ...
        void Hub_UpdateProduct(object sender, ProductUpdateEventArgs e)
        {
            try
            {
                Product product = _pruduct.FindByID(e.ProductID);

                txtSupplier.Tag = product.SupplierID;
                txtSupplier.Text = _supplier.GetSingle(new Supplier { SupplierID = product.SupplierID }).Name;

                txtCategory.Tag = product.CategoryID;
                txtCategory.Text = new CategoryService().GetSingle(new Category { CategoryID = product.CategoryID }).Name;

                txtBrand.Tag = product.BrandID;
                txtBrand.Text = new BrandService().GetSingle(new Brand { BrandID = product.BrandID }).BrandName;
        ...
        {
            List<ProductLocation> products = _prodLoc.FindAll("ProductID = " +productID).ToList();
        ...
        {
            //if (dgvLocation.RowCount > 1)
            //{
            //    for(int i = 1 ; i < dgvLocation.RowCount ; i++)
            //    {
            //        dgvLocation.Rows[i].ReadOnly = true ;
            //    }
            //}
        ...
        {
            cmbMeasurement.DataSource = new MeasurementService().GetAll();
