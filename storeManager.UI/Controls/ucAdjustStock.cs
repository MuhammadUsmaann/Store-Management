...
namespace storeManager.UI.Controls
{
    public partial class ucAdjustStock : UserControl
    {
        ...
        void Hub_ProductListAdded(object sender, ProductListAddedEventArgs e)
        {
            ...
            e.AddedRows.ToList().ForEach(p => products.Add(new ProductService().FindByID(p.ProductID.ToInt())));
                ...
                foreach (Product item in products)
                {
                    ...
                    if (locations.Count() > 1)
                    {
                        foreach (var itm in locations)
                        {
                            ...
                            location = new LocationService().GetSingle(new Location { Id = itm.LocationID });
                    ...
                    string locationName = new LocationService().GetSingle(new Location { Id = locationID }).Name;
        ...
        private void btnAdjust_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                for (int i = 0; i < dgvProducts1.Rows.Count; i++)
                {
                    ...
                    productLocation = locService.GetSingle(new ProductLocation { Id = prodLocID });
