...
namespace storeManager.UI.Controls
{
    public partial class ucQuote : UserControl
    {
        ...
        private SaleDetailService _saleDetail;
        private SaleService _sale;
        private ProductService _product;
        private TaxService _tax;
        private PaymentModeService _paymentMode;
        ...
        {
            Sale sale = _sale.GetSingle(new Sale { SaleID = e.SaleID });

            cmbLocation.SelectedValue = new CompanyService().GetAll().FirstOrDefault().DefaultLocation;
            txtCustomer.Text = _customer.GetSingle(new Customer { CustomerID = int.Parse(sale.CustomerID) }).CustomerName;
            txtCustomer.Tag = sale.CustomerID;
            txtSalesPerson.Text = new UserService().FindByID(int.Parse(sale.EmployeeID)).UserName;
        ...
        private void Init()
        {
            ...
            company = new CompanyService().GetAll().FirstOrDefault();
        }
        ...
        private void btnRecord_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    sales = _sale.GetSingle(new Sale { SaleID = _sale.Add(sales) });
                }
                catch (Exception ex)
                {
        ...
        {
            cmbPaymentMode.DataSource = new PaymentModeService().GetAll();
        ...
        {
            IEnumerable<Location> location = new LocationService().GetAll().Where(l => l.Id == company.DefaultLocation).ToList();
        ...
        private void txtBarCode_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    Product product = _product.FindAll("Barcode = "+ txtBarCode.Text).FirstOrDefault();
