    ...
    public partial class ucSale : UserControl
    {
        ...
        void Hub_EditSale(object sender, EditSaleEventArgs e)
        {
            try
            {
                ...
                Sale sale = _sale.GetSingle(new Sale { SaleID = _voidSaleID });
                Customer customer = _customer.GetSingle(new Customer{CustomerID = int.Parse(sale.CustomerID)});
                User user = new UserService().FindAll("EmployeeID = " + int.Parse(sale.EmployeeID)).FirstOrDefault();
        ...
        private void Init()
        {
            ...
            _tax = new TaxService();
            _company = new CompanyService().GetAll().FirstOrDefault();
        ...
        {
            IEnumerable<Location> location = new LocationService().GetAll().Where(l => l.Id == _company.DefaultLocation).ToList();
